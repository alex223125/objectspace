class SharedClassLayer::InterfaceGroupsController < ApplicationController
  ...
  def destroy
    binding.pry
    set_redirect_after_destroy_path
    @interface_group.destroy

    binding.pry
    respond_to do |format|
      format.html { redirect_to @redirect_after_destroy_path, notice: "Actions group was successfully deleted." }
  ...
  def set_redirect_after_destroy_path
    #DOC: 1.Case one when interface group inside another interface group
    parent_interface_group = @interface_group.parent
    if parent_interface_group.present?
      @redirect_after_destroy_path = interface_group_path(ownername: parent_interface_group.closest_simple_class.ownerable.ownername,
                                                          id: parent_interface_group.slug)
    else
      #DOC: 2.Case two when interface group located in class container
      class_container = @interface_group.class_container
      @redirect_after_destroy_path = class_container_path(ownername: class_container.closest_simple_class.ownerable.ownername,
    ...
    def set_redirect_to_upper_structure_path
      ...
      if @interface_group.parent.present?
        ...
        if parent_interface_group.regular_functional_type? || parent_interface_group.class_container_root_functional_type?
          ...
          binding.pry
          class_layer_entity = parent_interface_group.class_layer_entity
          if class_layer_entity.class == SimpleClasses::SimpleClass
            @redirect_to_upper_structure_path = simple_class_path(ownername: class_layer_entity.owner.ownername,
                                                                  id: class_layer_entity.slug)
          elsif class_layer_entity.class == Frameworks::Framework
            @redirect_to_upper_structure_path = framework_path(ownername: class_layer_entity.owner.ownername,
                                                                  id: class_layer_entity.slug)
          end
        end
